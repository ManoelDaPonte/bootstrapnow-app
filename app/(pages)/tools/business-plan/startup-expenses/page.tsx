...
const StartupFinancialDashboard: React.FC = () => {
	...
	const addNewCategory = useCallback(() => {
		...
		const newCategoryId = newCategory.name
			.toLowerCase()
			.replace(/\s+/g, "-");
	...
	const addRow = useCallback(
		(section: "investors" | "loans" | "expenses") => {
			...
			setFinancialData((prev) => {
				const sectionKey =
					section === "expenses" ? "expenses" : "capital";
					...
					[sectionKey]: {
						...
						[section]: [
							...currentEntries,
							{
								...
								type: "general",
								category:
									prev.categoryDefinitions[section][0]?.id ||
									"",
							},
						],
					},
	...
	const removeRow = useCallback(
		(section: "investors" | "loans" | "expenses", id: string) => {
			setFinancialData((prev) => {
				const sectionKey =
					section === "expenses" ? "expenses" : "capital";
				const currentEntries =
									...
									{Object.entries({
										...
										<Button
											...
											onClick={() =>
												addRow(
													section as
														| keyof StartupFinancialData["capital"]
														| "expenses"
												)
											}
										>
														...
														>
															...
															<SelectContent>
																{financialData.categoryDefinitions[
																	section as keyof StartupFinancialData["categoryDefinitions"]
																].map((cat) => (
																	<SelectItem
																		key={
																			cat.id
									...
									{financialData.risks.map((risk, index) => {
										...
										return (
											<TableRow key={index}>
												...
												>
													{risk.probability.toFixed(
														1
													)}
													%
												</TableCell>
												<TableCell
													className={textColorImpact}
												>
													{risk.impact.toFixed(1)}
												</TableCell>
												<TableCell
													className={`${backgroundColor} text-white font-bold px-4 py-2`}
												>
													{score}
												</TableCell>
												<TableCell>
